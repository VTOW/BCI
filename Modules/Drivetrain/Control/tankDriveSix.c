...
#ifndef TANKDRIVESIX_C_INCLUDED
#define TANKDRIVESIX_C_INCLUDED

#ifdef SIX_MOTOR_TANK_DRIVE

#include "tankDriveSix.h"
#include "..\..\Core\motorControl.c"
#include "..\..\Core\joystickDefinitions.h"

#define setAllDriveMotors(power) setMotorSpeed(drive.leftTop, power); setMotorSpeed(drive.leftMiddle, power); setMotorSpeed(drive.leftBottom, power); setMotorSpeed(drive.rightTop, power); setMotorSpeed(drive.rightMiddle, power); setMotorSpeed(drive.rightBottom, power)
#define setLeftDriveMotors(power) setMotorSpeed(drive.leftTop, power); setMotorSpeed(drive.leftMiddle, power); setMotorSpeed(drive.leftBottom, power)
#define setRightDriveMotors(power) setMotorSpeed(drive.rightTop, power); setMotorSpeed(drive.rightMiddle, power); setMotorSpeed(drive.rightBottom, power)

#define setAllDriveMotorsRaw(power) motor[drive.leftTop] = power; motor[drive.leftMiddle] = power; motor[drive.leftBottom] = power; motor[drive.rightTop] = power; motor[drive.rightMiddle] = power; motor[drive.rightBottom] = power
#define setLeftDriveMotorsRaw(power) motor[drive.leftTop] = power; motor[drive.leftMiddle] = power; motor[drive.leftBottom] = power
#define setRightDriveMotorsRaw(power) motor[drive.rightTop] = power; motor[drive.rightMiddle] = power; motor[drive.rightBottom] = power

#if defined(DRIVE_USING_QUADRATURE_ENCODERS)
#ifdef DRIVE_USING_GYRO

//Drive representation
typedef struct sixMotorTankDrive
{
	driveMotor *leftTop;
	driveMotor *leftMiddle;
	driveMotor *leftBottom;
	driveMotor *rightTop;
	driveMotor *rightMiddle;
...
static sixMotorTankDrive drive;
...
#elif defined(DRIVE_USING_INTEGRATED_MOTOR_ENCODERS)
#ifdef DRIVE_USING_GYRO
...
void setDrive(driveMotor *leftTop, driveMotor *leftMiddle, driveMotor *leftBottom, driveMotor *rightTop, driveMotor *rightMiddle, driveMotor *rightBottom, tSensors leftQuad, tSensors rightQuad, tSensors gyro, int joyThreshold)
{
	...
	drive.leftMiddle = leftMiddle;
	drive.leftBottom = leftBottom;
	drive.rightTop = rightTop;
	drive.rightMiddle = rightMiddle;
...
task driveJoystickControl()
{
	...
	#endif //#ifdef USING_TANK_DRIVE

	#ifdef USING_ARCADE_DRIVE
	...
	#endif //#ifdef USING_ARCADE_DRIVE
}

#endif //#ifdef SIX_MOTOR_TANK_DRIVE

#endif //#ifndef TANKDRIVESIX_C_INCLUDED
